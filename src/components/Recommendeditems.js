...
class Recommendeditems extends Component {
    constructor(props, context) {
        super(props, context);
        this.state = {
            loadAjaxStatus: false, loadRecommendListDisplay : false, manageRecommendlist:'', recommendList :[], productViewStatus: false, viewProductscroll:false, setDeleteMessage: false, count:localStorage.getItem('recommendCount'),  copied: false, socialMedia: false, showRecommendedList:props.showRecommendedList, setMessage:'',
			sort:'desc', deleteProdID:'', deleteKey:'', deleteUserID: '', showProductDetails: '', showProductDetailsSms:'', message: { to: '', body: ''}, submitting: false, error: false, error_response:'', enableRecDelete: false
        };   
		this.OutsideClickforDetails = this.OutsideClickforDetails.bind(this); 
		//this.onHandleChange = this.onHandleChange.bind(this);
		//this.onSubmit = this.onSubmit.bind(this);
        $('.recommended_wrap .manageSmsForm').removeClass('active');			
    }
    	
	scrollToSection = () => {			
		var st = $('.recommended_list .acc-scroll-wrap > div').scrollTop();		
		setTimeout(() => {	
			if (st > lastScrollTop){
    ...
    componentDidMount() {	
	 if(this.props.recommendedListactive === true){
		 this.setState({productViewStatus:false});
	 }
	     			   
		let user = (localStorage.getItem('userDetails') != 'undefined'?JSON.parse(localStorage.getItem('userDetails')):'');  
		if(user !=null && user.user_fname != undefined && this.props.recommendedListactive === true){
		 var store_list = this.loadRlists(user.user_id);
	    }
		else{
		 var store_list = this.noResultFound();	
	    }	 
		this.setState({ manageRecommendlist:store_list});
		
    }
	
	
     componentWillReceiveProps(recommendProps) {	
	 	if(this.props.recommendedListactive === true){
		 this.setState({productViewStatus:false});		
	    }
		...
		if(user != null && user.user_id != undefined && this.props.recommendedListactive === true && this.props.recommendedListactive == recommendProps.recommendedListactive){
		 var store_list = this.loadRlists(user.user_id);
	    }
		else{
		 var store_list = this.noResultFound();	
	...
	deleteItemSelected(product_id) {
	  this.setState({enableRecDelete:true},()=>{
		  $('.delete-select-remove').attr('style', 'font-family: '+ this.props.mainFontFamily +' !important');						
	  });
		if ($('.recommend-row#grid_'+product_id+ '.selected')[0]){
		   $('.recommend-row#grid_'+product_id).removeClass('selected');
		} else{
		   $('.recommend-row#grid_'+product_id).addClass('selected');
		}		
	}	
	multiDeleteItem(){
   ...
   deleteProductPrompt() {
    let user = (localStorage.getItem('userDetails') != 'undefined'?JSON.parse(localStorage.getItem('userDetails')):''); 	   
     var user_id = user.user_id;   
		...
		$('.recommended_wrap .confirm-action').addClass('show');
	}	
	
    deleteProduct(val,user_id) {
	 $('.recommended_wrap .confirm-action').removeClass('show');
	 if(val == 'no')
	 return;
		...
		var code = 'grid_0';
		var recIds = [];
		 $(".recommend-row.selected").each(function() {
				code = $(this).attr('id');
				code = code.split('_');
				recIds.push(code[1]);
				$('.recommend-row#grid_'+code[1]).addClass('delete-row');
				setTimeout(() => {
				   $('.recommend-row#grid_'+code[1]).hide();
				},100);
         });
	
		setTimeout(() => {
			axios.post(APIURL+'products/deleteRecommends', qs.stringify({user_id: user_id,product_id:recIds})).then(response => {
			if(response.data.status === "ok"){	
           	    var store_list_delete = this.loadRlists(user_id);
			    this.setState({ manageRecommendlist:store_list_delete, productViewStatus: false, enableRecDelete: false});			    
				...
				$('.recommend-row').removeClass('delete-row'); 
			}
	...
	onCopy = () => { 
	 ...
	 loadSmsForm = (shareLink, productID) => {
		...
		setTimeout(() => {
			$('.recommended_wrap .manageSmsForm').addClass('active');
		    $('#share_link_recommend').val(shareLink);
	    }, 100);
		setTimeout(() => {
			$('.recommended_wrap .product-detail-content').removeClass('active'); 
	    }, 500);
   ...
   smsCloseWish = () =>{
	   $('.recommended_wrap .manageSmsForm').removeClass('active');
	   setTimeout(() => {
		     $('.recommended_wrap .product-detail-content').addClass('active'); 
  ...
  onHandleChange(event) {
	...
	var getShareLink = '';
	if(document.getElementById("share_link_recommend")){
	getShareLink = document.getElementById("share_link_recommend").value;
	}
   ...
   showProductDetailsSms(updatedTotalRes='') {
    ...
    loadProduct(masterID, viewType = '') {
		...
		this.props.changeStatus(true, 'Detail');
	 	axios.post(APIURL+'products/productRecommendDetails', qs.stringify({masterID:masterID })).then(response => {
			if(response.data.status === "ok"){
	...
	productDetailsScroll =() =>{	
		$(".recommended_list .acc-scroll-wrap > div").animate({ scrollTop: 0 }, 600);	
		return false;	
	}
      ...
      if (this.node.contains(e.target)) {
       return;
	  	...
	  	var uri = window.location.toString();
			...
			this.props.changeStatus(false,'UserLikes');
		$('.recommended_wrap .product-detail-content').removeClass('active'); 
		$('.recommended_wrap .manageSmsForm').removeClass('active');
	  } 	
   ...
   loadRlists = (user_id) => {	
    $('.recommended_wrap .confirm-action').removeClass('show');
  	   if(user_id != undefined){	
	   	this.setState({loadAjaxStatus : true},()=>{ $('.load-text').attr('style', 'font-family: '+ this.props.mainFontFamily +' !important');});
		axios.post(APIURL+'products/recommendDetails', qs.stringify({user_id:user_id})).then(response => {
			if(response.data.status === "ok"){		
			    this.setState({recommendList:response.data.result, count:response.data.count, setMessage:'', loadRecommendListDisplay:'', loadAjaxStatus : false});
			    localStorage.removeItem('recommendCount');     				
     			localStorage.setItem('recommendCount', this.state.count);
     			this.setState({loadRecommendListDisplay:this.loadRecommendList(response.data.result)}, () => {
					 $('.menu-toggle, .menu-toggle.logo-lg, .header-row-uxt a, .product_search_box input, .search-option h4, .minmax-filter span,  .minmax-filter label, .company-name p a, .pdt-price .price, .result_count span, .dtls-txt .item-name, h3.price, .amazon, .acs_control_sec h2, .acs_control_sec span, .acs_control_sec p, .accessiblity-wrap h3, .ac_buy_btn, .tag-input, .topfull-close, .close-icon, .package_tabnav li, .open-package .open-login, .profile-edit, .form-group, .form-control, .forgot-pwd, .alert-success, .alert, .alert-track, .notify, .downgrade-action, .pro-title, .clear-selection, .acc-package-desc center b, .custom-select-box, .cus-select-search input, .cus-select-options ul li, .tnk-header h2, .clear-search, .relevance, .btn, .copy-text, .upgrade-remainder p, .no-record, .relevantList h4, .relevantList .relevantList h4, .relevantList p, .manageSmsForm, .sf-title, .minmax-filter, .error_sms, .recent-products-grid p, .recommended-products-grid p, .recent-products-grid p a, .recommended-products-grid p a, .delete-select-remove, .load-text').attr('style', 'font-family: '+ this.props.mainFontFamily +' !important');			
			...
			setTimeout(function(){ 		
				$('.recommend-row').removeClass('delete-row'); }, 100);				
			}
			else{
			 setTimeout(function(){ 
			  $('.recommend-row').removeClass('delete-row');
			 }, 100);
			 this.setState({setMessage:response.data.message, count:response.data.count, loadRecommendListDisplay:false});
			 localStorage.removeItem('recommendCount');     				
     		 localStorage.setItem('recommendCount', this.state.count);
     		}
		 }).catch(function (error) {
	...
	loadRecommendList(recommendList) {
	 	var current = this;		
		var user = (localStorage.getItem('userDetails') != 'undefined'?JSON.parse(localStorage.getItem('userDetails')):''); 
		const listItems =  Object.keys(recommendList).map(function(index){
			var currentProduct = current.state.recommendList[index]['main'];
			var masterID = current.state.recommendList[index]['recommend_id'];			
			var updatedProduct = current.state.recommendList[index]['new'];
			var recommended_user = current.state.recommendList[index]['user_id'];
			...
			newPriceAvailable = (newPriceAvailable > 0?newPriceAvailable.toFixed(2):newPriceAvailable);
		    actualPrice = (actualPrice >0?actualPrice.toFixed(2):actualPrice);
			var setUnikey = 'recommend'+index;
			var storeUPC = (currentProduct.upc?currentProduct.upc:currentProduct.sku);
			 if(currentProduct.name!=null) {			 
		 		return (	
		 		  	 <div key={setUnikey} className="comparison-list-row recommend-row" id={'grid_'+masterID}>	  
					...
					<div className="product-dtls-bottom">	 
						 ...
						 <div className="pdt-price">
							...
							{recommended_user == user.user_id?<Link className="close-icon" to='#' onClick={current.deleteItemSelected.bind(current,masterID)}>X</Link>:''}
							
							{/*recommended_user == user.user_id?<Link className="close-icon" to='#' onClick={current.deleteProductPrompt.bind(current,masterID,user.user_id,index)}>X</Link>:''*/}
						 </div>	
					</div>			
				 </div>
	...
	noResultFound(){
		 return (
			<div className="notify">Please login to view the recommended items.</div>
			);			
	}
	...
	manageProdView(){	
	  ...
	  $('.recommended_wrap, .recommended_wrap .product-detail-content, .recommended_wrap, .recommended_wrap .product-detail-content').removeClass('active');
    ...
    removeSelection(){
		this.setState({enableRecDelete:false});
		$('.recommend-row').removeClass('selected');
	}
   ...
   componentDidMount() {		
	...
	$('.recommended_wrap .product-detail-content').removeClass('active');
   	...
   	sortWishlistProducts = (sort_type) => {
	   	this.setState({loadAjaxStatus : true},()=>{ $('.load-text').attr('style', 'font-family: '+ this.props.mainFontFamily +' !important');});
		  var search_results = this.state.recommendList;
		  if(sort_type === 'asc'){
			 ...
			 search_results.sort(function(a, b){ return b.main.price - a.main.price });
			   ...
			   this.setState({sort:set_sort, recommendList:search_results, loadAjaxStatus : false, loadRecommendListDisplay:this.loadRecommendList(search_results)});  	
       }
    ...
    render() {
		...
		if(user != null && user.user_id != undefined){
		 var manageDisplay = this.state.loadRecommendListDisplay;
		}
		else{
		 var manageDisplay = this.state.manageRecommendlist;
		}	
		
		 if(this.state.msgSuccess===true && this.state.setDeleteMessage)
		 {
			 setTimeout(function(){ $('.recommend-row').removeClass('delete-row');
				}, 500);
		 }
     ...
     var user_id = this.state.deleteUserID;
		...
		{((this.state.productViewStatus === true && this.state.loadAjaxStatus===true) || (this.state.loadAjaxStatus===true && this.props.recommendedListactive===true))?
		<div className="uxt-loading dynamic-loader rl-loader">
	<div className="loading-inner"><span className="load-text">{this.props.loaderText}</span>{(this.props.loaderImage !== '')? <img src={this.props.loaderImage} alt=""/> : ''}		
        ...
        <div className={(this.props.showRecommendedList===true)?'recommended_wrap active':'recommended_wrap'}>		
			
			<div className={((this.state.productViewStatus===true && this.props.showRecommendedList===true)?'product-detail-content active':'product-detail-content')+(this.props.topToBottom == true?' top-view':' right-view')} ref={node => { this.node = node; }}>
				...
				<h4 className="title">Send a text message of this find</h4>
				  <form onSubmit={this.onSubmit.bind(this)} className={this.state.error ? 'error sms-form' : 'sms-form'}>
					...
					<div className="form-group">
					 ...
					 <input
						type="hidden"
						name="share_link_recommend"
						className="form-control"
						id={"share_link_recommend"}
					  />								  
					</div>
			...
			<div className="confirm-action">
				<p>Are you sure you want to delete the Recommended items?</p>
			...
			<div className={(this.props.recommendedListactive===true || this.props.showRecommendedList===true)?'recommended_list active':'recommended_list'}>						
			...
			{user != null && user.package_plan != undefined && user.package_plan == 'free'?<p className='alert-track'>Upgrade to PRO for accessing price drop/up of items saved as Recommended.</p>:''}
		
			<div className='rearrange-result wishlist-rearrange fixedCls'>				
			<div className="result_count">
			<span>{localStorage.getItem('recommendCount')} User {localStorage.getItem('recommendCount') == 1?'Like':'Likes'}</span>	
 ...
 <g>
  ...
  <path d="m584.47 22.398h-220.47v515.2h220.47c6.1328 0 12.016-2.4375 16.352-6.7773 4.3398-4.3359 6.7773-10.219 6.7773-16.352v-468.95c0-6.1328-2.4375-12.016-6.7773-16.352-4.3359-4.3398-10.219-6.7773-16.352-6.7773z"/>
				...
				{this.state.enableRecDelete == true?<span className={(this.state.sort=='asc')?'multi-delete-icon active':'multi-delete-icon'} onClick={this.multiDeleteItem.bind(this)}>
...
<svg width="700pt" height="700pt" version="1.1" viewBox="0 0 700 700" xmlns="http://www.w3.org/2000/svg">
 ...
 <g>
  ...
  <path d="m463.16 140.01h-66.488v-11.676h-93.336v11.676h-66.516c-1.9141 0-3.4805 1.5664-3.4805 3.4805l-0.003907 16.344c0 1.9336 1.5664 3.5078 3.4805 3.5078h226.34c1.9414 0 3.5078-1.5781 3.5078-3.5078v-16.344c0.003907-1.9102-1.5664-3.4805-3.5078-3.4805z"/>
				...
				{this.state.enableRecDelete == true?<a className="delete-select-remove" href='javascript://' onClick = {this.removeSelection.bind(this)}>Clear</a>:''}
				</div>		
